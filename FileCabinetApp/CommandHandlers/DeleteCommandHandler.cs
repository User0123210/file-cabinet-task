    ...
    public class DeleteCommandHandler : ServiceCommandHandlerBase
    {
        ...
        private void ParseArguments(string[] arguments)
        {
            if (arguments.Length > 1)
            {
                ...
                {
                    string[] propertyValue = criteria[j].Split("=", 2);
                ...
                {
                    if (this.CheckIfToDelete(criteria, record))
                    {
        ...
        private bool CheckIfToDelete(string[] criteria, FileCabinetRecord record)
        {
            bool toDelete = true;

            for (int i = 0; i < criteria.Length; i++)
            {
                string[] propertyValue = criteria[i].Split("=", 2);
